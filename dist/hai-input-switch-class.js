...
class HaiInputSwitch extends HaiInput
{
    ...
    variant = 'on/off';
    /** @type {string} - Display orientation (can be 'horizontal' or 'vertical'). Only used with 'multiple' variant.*/
    ...
    processParameters(parameters = null)
    {
        ...
        {
            if(['horizontal', 'vertical'].includes(parameters.orientation) === false)
            {
                console.warn('HaiForm: Parameter "orientation" can be either "horizontal" or "vertical", reverting back to default.');
            }
            else
            {
